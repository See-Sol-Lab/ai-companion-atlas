...
if (primaryExplore) primaryExplore.innerHTML = 'Explore projects <span aria-hidden="true">↗</span>';
...
const CATEGORY_FILTERS = {
  memory: 'Memory',
  subjectivity: 'AI Agency',
  senses: 'Senses & Embodiment',
...
if (taxonomySection && directorySection) {
  ...
  quickFilters.innerHTML = `
    <button class="atlas-quick-filter" type="button" data-catalog-filter="high-star" aria-pressed="false">
      <span class="atlas-quick-filter-title">Popular on GitHub</span>
      <span class="atlas-quick-filter-subtitle">500+ GitHub stars</span>
    </button>
    <button class="atlas-quick-filter" type="button" data-catalog-filter="editor-pick" aria-pressed="false">
      <span class="atlas-quick-filter-title">Editor picks</span>
      <span class="atlas-quick-filter-subtitle">Selected projects</span>
    </button>
  `;
  if (capabilityStrip) {
...
if (directorySection && projectGrid && projectCards.length > 0) {
  ...
  pagination.innerHTML = `
    <button class="catalog-page-button catalog-page-prev" type="button" aria-label="Previous">← <span>Previous</span></button>
    <div class="catalog-page-numbers" aria-label="Page numbers"></div>
    <span class="catalog-page-mobile-status" aria-live="polite"></span>
    <button class="catalog-page-button catalog-page-next" type="button" aria-label="Next"><span>Next</span> →</button>
  `;
  ...
  const syncDirectoryHeading = (matchCount) => {
    if (searchQuery) {
      ...
      directoryFilterNote.textContent = `${matchCount} matching projects`;
      directoryFilterNote.hidden = false;
      return;
    }

    const filteredHeading = {
      'high-star': ['Popular on GitHub', 'GitHub Star ≥ 500'],
      'editor-pick': ['Editor picks', 'Community favorites']
    }[catalogFilter] || (CATEGORY_FILTERS[catalogFilter] ? [`${CATEGORY_FILTERS[catalogFilter]} projects`, 'Category'] : null);
    ...
    directoryTitle.innerHTML = `<strong>${projectCards.length}</strong><span> projects · </span><strong>${projectAuthorCount}</strong><span> authors</span>`;
  ...
  const renderPage = (requestedPage, { historyMode = 'push', scroll = false } = {}) => {
    ...
    emptyState.textContent = searchQuery ? 'No matching projects. Try another search.' : 'No projects in this category yet.';
