...
const publicOrigin = 'https://www.ailover-atlas.com';
...
<head>
  ...
  <link rel="canonical" href="${publicOrigin}/projects/${escapeHtml(project.slug)}/" />
...
const publicPaths = [
  '/',
  '/deepseekgui/',
  '/deepseekgui/download/',
  '/deepseekgui/docs/',
  '/deepseekgui/privacy/',
  ...projects.map((project) => `/projects/${project.slug}/`)
];
const sitemapUrls = publicPaths
  .map((pathname) => `  <url><loc>${publicOrigin}${pathname}</loc></url>`)
  .join('\n');
await writeIfChanged(
  sitemapPath,
